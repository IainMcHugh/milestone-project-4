...
def cache_checkout_data(request):
    ...
    try:
        ...
        messages.error(
            request,
            "Sorry, your payment can't be processed right now. Please try again later.",
        )
    ...
    if request.method == "POST":
        ...
        if order_form.is_valid():
            ...
            for item_id, item_data in cart.items():
                try:
                    product = Product.objects.filter(soft_delete=False).get(id=item_id)
                    ...
                    messages.error(
                        request,
                        "One of the products in your cart wasn't found in our database. Please call us for assistance.",
                    )
                    order.delete()
            ...
            messages.error(
                request,
                "There was an error processing your order. Please re-enter your information",
            )
    else:
        cart = request.session.get("cart", {})
        if not cart:
